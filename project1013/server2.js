...
var server=http.createServer(function(request, response){
    ...
    if(parseObj.pathname=="/member/registForm"){//회원가입 양식 디자인 폼을 원하면
        ...
        con.query(sql, [param.uid,param.password,param.uname,param.phone,param.email,param.receive,param.addr,param.memo], function(error,result,fields){
            if(error){
                console.log("등록실패",error);
            }else{
                var msg="<script>";
                msg+="alert('가입성공')";
                msg+="location.href='/member/list';";//<a href="">
                // /member/list/로 재접속(클라이언트가 지정한 주소로 재접속)
                msg+="</script>";
        ...
        con.query(sql,function(error,record,fields){
            if(error){
                ...
                for(var i=0;i<record.length;i++){
                    var json=record[i];//배열의 각 요소마다 회원정보 json호출
                    tag+="<tr>";
    ...
    }else if(parseObj.pathname=="/member/edit"){//회원정보수정을 원하면
