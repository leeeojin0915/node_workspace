...
//http 메카니즘이다. 만일 응답을하지않으면? 클라이언트는 무한정 서버의 응답을 기다리므로
//무한대기상태에 빠진다.
//서버객체를 생성
...
var server=http.createServer(function(request, response){
    ...
    //response 객체로는 클라이언트에게 전송 할 응답 정보를 구성할 수 있다.
    console.log("클라이언트 요청을 받았습니다.");

    //컨텐츠 전송(클라이언트의 브라우저가 받게 될 내용)
    //HTTP 상태코드 중 200은 정상 처리를 의미.(즉, 서버에서 클라이언트의 요청을 정상정으로 처리했다는 상태코드)
    //누가 정한건가? W3C 표준에 의해 정해진 것
    //참고 500 심각한 서버의 에러, 404 요청한 자원을 찾을 수 없을때..
    response.writeHead(200, {"Content-Type":"text/html;charset=utf-8"}); //편지봉투구성하기
    /*
    var tag=""; 
    tag+="<html>";
    tag+="<head>";
   ...
   */
   ...
   //참고로 이미지의 경우 content-type은 image/png, image/jpeg, image/gif등
   /*
