...
var http=require("http");//http모듈가져오기
var fs=require("fs"); //file system 모듈 가져오기
//http모듈로 부터 server객체 생성하기
var server=http.createServer(function(request,response){
    ...
    //1)클라이언트가 회원가입 양식을 보기를 원하면 아래의 코드가 수행되야함
    //서버에 존재하는 회원가입양식 폼파일을 읽어서 클라이언트의 브라우저에 보내기
    ...
    //2)가입을 원할 경우에 db에 넣어줘야 하고
    //3)이미지를 원하는 이미지 보여주기
    //4)오디오파일을 원하면 오디오를 전송
});
